...
func Test_scaling_change_setting(t *testing.T) {
	err := k8sutil.ES_change_settings(NonExistingIP, "1m", "request"); 
	if (err == nil){
...
func Test_check_for_green(t *testing.T) {
	err := k8sutil.ES_checkForGreen(NonExistingIP); 
	if (err == nil){
...
func Test_check_for_nodeUp(t *testing.T) {
	err := k8sutil.ES_checkForNodeUp(NonExistingIP, NonExistingPOD, 5); 
	if (err == nil){
